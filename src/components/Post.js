...
const mdxComponents = {
	...
	'pre.code': ({ className, ...props }) => <code className={`${className} text-gray-200`} {...props} />
};
	...
	return (
		<article className="xl:divide-y xl:divide-gray-200">
			<Head>
				...
				{meta.tags.map((tag, i) => <meta key={`tag-${i}`} property="article:tag" content={tag.name} />)}
				...
				<script
					...
					dangerouslySetInnerHTML={{ __html: JSON.stringify(makeJSONSchema(meta, router.pathname)) }}
				/>
				{/* {isAmp && (
					<style jsx global>
						{globalStyles}
					...
					{meta.repo && (
						<div class="bg-white shadow sm:rounded-lg border-l-4">
							<div class="px-4 py-5 sm:p-6">
								<h3 class="text-lg leading-6 font-medium text-gray-900">Source Code Available</h3>
				...
				<footer className="text-sm font-medium leading-5 divide-y divide-gray-200 xl:col-start-1 xl:row-start-2">
					...
					{(next || previous) && (
						<div className="space-y-8 py-8">
							{next && (
								<div>
									<h2 className="text-xs tracking-wide uppercase text-gray-500 mb-2">Next Article</h2>
